...
public class ConditionType<T> {
    ...
    public ConditionFactory<T>.Instance read(JsonElement jsonElement) {
        if(jsonElement.isJsonObject()) {
            ...
            Optional<ConditionFactory<T>> optionalCondition = Optional.ofNullable(conditionRegistry.get(type));
            if(!optionalCondition.isPresent()) {
                throw new JsonSyntaxException(conditionTypeName + " json type \"" + type + "\" is not defined.");
            }
